=== Abundatrade Plugin ===
Plugin Name: Abundatrade Plugin
Contributors: withinboredom
Donate Link: http://withinboredom.info/?product=donate
Tags: plugin,post,page,abundatrade,affiliate,books,cds,dvds,used,cash
Requires at least: 3.3
Tested up to: 3.5
Stable tag: 1.7.7

Earn extra income for your site via the Abundatrade affiliate program.  The shortcode puts the calculator on your website that shows visitors how much cash they will get for their used CDs, DVDs, and books.

== Description ==

Abundtrade pays cash for used books, CDs, DVDs, and video games.   Abundtrade affiliates can earn extra cash by referring customers to Abundtrade that send in their used goods for cash.    Now you can tap into this lucrative market by using the Abundatrade Plugin!

Install the plugin on your WordPress site and show your site visitors how much cash they can earn for their used goods via the inline trade calculator.  Visitors are given an interactive form where they can enter a UPC or ASIN and it will populate an inline table showing the cash value for the item(s) they enter.

As an affiliate you can set the percentage that you want to earn on the Abundtrade cash value for the trade.  You can use the Abundtrade relationship as a traffic builder and set your earnings percentage at 0% which offers your visitors the highest value for their trade.  Any percentage over 0% up to 25% becomes an income earner for your site.

== Features ==

= Custom Styling =

= Related Links =

* [Abundatrade.com](http://www.abundatrade.com) 

== Installation ==

= Requirements =

PHP 5.2 or later.

= Automated Installation =

 * The fastest way to install is to use the WordPress admin panel, search for Abundatrade, and install the plugin from there.
 
= Manual Installation = 

 * Download the zip file.
 * Go to your WordPress admin panel and select add new plugin, upload, locate your zip file.
 * Enter your Abundtrade affiliate ID.
 * Enter the [abundatrade] on any page you want to show the calculator on.
 

== Frequently Asked Questions ==

= Can I add custom themes? =

Absolutely! Use the template in 
/wp-content/plugins/abundatrade-plugin/themes/classic.css and copy it 
to /wp-content/uploads/abundatrade/themes/your-name.css and edit away...

= Where can I find support for this plugin? =

Please use the forums on the plugin pages

= How can I translate the plugin into my language? =

* Find on internet the free program POEDIT, and learn how it works.
* Use the .pot file located in the languages directory of this plugin to create or update the .po and .mo files.
* Place these file in the languages subdirectory.
* If everything is ok, email the files to landers.robert@gmail.com and we will add them to the next release.
* For more information on POT files, domains, gettext and i18n have a look at the I18n for WordPress developers Codex page and more specifically at the section about themes and plugins.

== Screenshots ==

== Changelog ==

= v1.7.7 (Feb 18, 2013) =

* Minor bug fixes
* Abundagadgets short code added
* Contact form added

= v1.7.6 (Feb 2, 2013) =

* Hide $0 value items with a show/hide toggle

= v1.7.5 (Jan 31, 2013) =

*Send encrypted password on signin

= v1.7 (Jan 28, 2013) =

* Fixed javascript errors

= v1.6.7 (Jan 15, 2013) =

* Fixed file modes

= v1.6.6 (Jan 15, 2013) =

* Better gadget handling

= v1.6.5 (Jan 10, 2013) =

* Added better javascript support
* Now can submit error reports
* Featured gadgets

= v1.6.1 (December 21, 2012) =

* Now can add custom themes to wp-content/uploads/abundatrade/themes

= v1.6 (December 20, 2012) =

* Got a facelift!
* Misc. javascript errors fixed

= v1.5.1 (December 8, 2012) =

* Typo

= v1.5 (December 6, 2012) =

* Fixed registration so to be a better user experience
* Better recognition of products that are the same but using different codes
* Minified javascript for faster load times

= v1.3 (November 14, 2012) =

* Added guest submissions
* Fixed misc. bugs

= v1.2 (November 13, 2012) =

* Changed the layout of the bulk submission
* Added user support for login/registration and remember me info

= v1.1.1 (November 10, 2012) =

* Now will load themes for certain wp installations that would not before

= v1.1 (November 9, 2012) =

* Added theme selection

= v1.0.5 (November 8, 2012) =

* Fixed a bug that wouldn't allow ie8 users to submit a list

= v1.0.3 (November 5, 2012) =

* Reordered referrals

= v1.0.3 (November 7, 2012) =

* Fixed a javascript error on list submission
* Fixed a referral issue

= v1.0 (November 1, 2012) =

* Welcome out of beta!

= v0.7.1 (October 31, 2012) =

* Better verbage on bulk uploads

= v0.7 (October 31, 2012) =

* Properly save settings

= v0.6 (October 31, 2012) =

* Better bulk upload handling

= v0.5.4 (October 29, 2012) =

* Added bulk upload support

= v0.5.3 (October 29, 2012) =

* Added bulk upload support

= v0.5.2 (October 25, 2012) =

* Fixed a js bug

= v0.5.1 (October 25, 2012) =

* Fixed a js bug

= v0.5 (October 25, 2012) =

* Added support for small bulk uploads

= v0.1 (October 23, 2012) =

* Initial Release

= v0.2 (October 24, 2012) =

* Changed some meta data and removed some debug code
